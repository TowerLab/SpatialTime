...
fracture <- readRDS("D10_young_DRG_mapped.rds")
View(fracture@meta.data)

is(fracture, "Seurat")
Idents(fracture) <- fracture$DRG_status
...
subsetLabels(fracture, cluster = c("DRGpos"), export.all = T, dir.out = "DRG/") # Add error handling for terms that do not exist or poorly written
...
drg <- CoordMerge(file = "DRG/")
View(drg)

ltmr <- CoordMerge(file = "LTMR/")
View(ltmr)

all <- CoordMerge(file = "All/")
View(all)
...
# Example usage:
vis <- SpatialVis(fracture, sp, spatial.by = c("abs", "rel"), id = "DRG", slice = "slice1", return_obj = TRUE)
View(vis@meta.data)
...
SpatialFeaturePlot(fracture, features = "PEP_abs_slice1.3", images = "slice1.3")
