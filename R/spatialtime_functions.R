...
# Import dataset ----
fracture <- readRDS("fracture_original_annotation.rds")
is(fracture, "Seurat")
Idents(fracture)
...
subsetLabels("fracture_original_annotation.rds", cluster = c("MarrowFibrin", "Periosteum", "Cortical"),
             slice.n = "slice1", export.all = T, dir.out = "excel/") # Add error handling for terms that do not exist or poorly written
...
subsetLabels("fracture_original_annotation.rds", cluster = c("MarrowFibrin", "Periosteum", "Cortical"),
             slice.n = "slice1", export.all = T, dir.out = "excel/")
...
data <- SpatialCalc("fiji_output.csv", factor = 0.087658, colors = colors, tissue = tissue) # round up factor
View(data)
...
sp <- SpatialTime("excel/all_coordinates.csv", id = "MarrowFibrin") 
View(sp)
...
vis <- SpatialVis(fracture, sp, spatial.by = "rel", return_obj = F)
View(vis@meta.data)
...
ggplot(metadata) +
  geom_smooth(aes(st, HBA1), method = "loess", span = 1, se = F) +
  geom_smooth(aes(st, PDGFRA), method = "loess", span = 1, se = F) +
  theme_classic()
